const InventoryContainer = Vue.createApp({
  ...
  methods: {
    toggleSubmenu(submenu) {
      // Si el submenú ya está activo, lo cierra; de lo contrario, lo abre
    ...
    getInitialState() {
      return {
        ...
        activeSubmenu: null, // Puede ser 'give', 'drop', o null
        // Hotbar
        showHotbar: false,
        hotbarItems: [],
        // Notification box
    ...
    startDrag(event, slot, inventoryType) {
      ...
      // Escuchar eventos de movimiento y liberación en toda la ventana
    ...
    drag(event) {
      ...
      // Calcular nueva posición para el ítem
    ...
    endDrag(event) {
      ...
      // Limpiar datos del drag y listeners
    ...
    showContextMenuOptions(event, item) {
      event.preventDefault()
      const menuWidth = 240 // Ajusta según el ancho del menú contextual
      const menuHeight = 200 // Ajusta según la altura del menú contextual
      ...
      if (menuLeft + menuWidth > windowWidth) {
        menuLeft = windowWidth - menuWidth - 10 // Margen de 10px
      }
      if (menuTop + menuHeight > windowHeight) {
        menuTop = windowHeight - menuHeight - 10 // Margen de 10px
      }
